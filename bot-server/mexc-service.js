...
async function _pollClosedTrades(exchange, uid, db, bot, knownIds) {
  ...
  let trades = [];
  try {
    trades = await exchange.fetchMyTrades(undefined, undefined, 50);
  } catch (e) {
    // Если нет прав или временная ошибка — пропускаем
    console.warn(`[MexcService] fetchMyTrades warn uid=${uid}:`, e.message);
    return;
  }
